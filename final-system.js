...
/* ================================================================
   NEW FEATURES PATCH — OutfitKart v7.0
   ================================================================
   1. Strict Gender Filter (Women ke products Men mein nahi)
   2. Duplicate Image Auto-detect + Remove
   3. Combo PDP Layout (vertical thumbnails + big preview)
   4. Dynamic Pricing — COD +₹19, UPI base price
   5. "You Might Also Like" section on PDP
   6. Popular Categories section on category page
   7. Smart Back Navigation (Category > Sub > PDP → Sub)
   ================================================================ */

(function _outfitkartNewFeaturesV7() {
  'use strict';

  /* ──────────────────────────────────────────────────────────────
     CSS for New Features
  ────────────────────────────────────────────────────────────── */
  (function _injectV7CSS() {
    ...
    s.textContent = `
      ...
      #ok-combo-thumb-col::-webkit-scrollbar { display: none; }
      .ok-combo-thumb {
      ...
      .ok-combo-thumb.active {
        border-color: #e11d48;
        transform: scale(1.05);
        box-shadow: 0 2px 8px rgba(225,29,72,0.3);
      }
      .ok-combo-thumb:hover { opacity: 0.85; }
      #ok-combo-main-img {
        flex: 1;
        border-radius: 12px;
        object-fit: cover;
      ...
      .ok-pt-btn .ok-pt-badge {
        font-size: 9px;
        font-weight: 600;
        color: #9ca3af;
      }
      .ok-pt-btn.active .ok-pt-badge { color: #e11d48; }
      .ok-cod-surcharge-note {
        font-size: 10px;
        color: #f59e0b;
        font-weight: 700;
        margin-top: 8px;
      ...
      .ok-ymla-scroll::-webkit-scrollbar { display: none; }
      .ok-ymla-card {
        flex-shrink: 0;
        width: 130px;
        cursor: pointer;
      ...
      .ok-ymla-card:active { transform: scale(0.97); }
      .ok-ymla-card img {
        width: 100%;
        height: 150px;
        object-fit: cover;
      ...
      .ok-popcats-scroll::-webkit-scrollbar { display: none; }
      .ok-popcat-item {
        flex-shrink: 0;
        display: flex;
        flex-direction: column;
      ...
      .ok-popcat-item:active .ok-popcat-img-wrap { transform: scale(0.93); }
      .ok-popcat-item:hover .ok-popcat-img-wrap { border-color: #e11d48; }
      .ok-popcat-img-wrap img {
      ...
      .ok-popcat-item .ok-popcat-img-wrap { position: relative; }
    `;
    document.head.appendChild(s);
  })();
  ...
  /* ──────────────────────────────────────────────────────────────
     ...
     Women's Watches category pe Men ke products nahi dikhne chahiye
  ────────────────────────────────────────────────────────────── */
  function _patchStrictGenderFilter() {
    if (window._genderFilterPatched) return;
    window._genderFilterPatched = true;

    // openSubcatProducts ko wrap karo
    const origOpen = window.openSubcatProducts;
    if (!origOpen) return;

    window.openSubcatProducts = function(cat, sub) {
      // Store current sub for smart back
      window._currentSubFilter = sub;
      window._currentCatFilter = cat;

      origOpen(cat, sub);

      // After render, filter products strictly by gender
      setTimeout(() => _enforceGenderFilter(cat, sub), 300);
      setTimeout(() => _enforceGenderFilter(cat, sub), 800);
    };
  }

  function _enforceGenderFilter(cat, sub) {
    const catLower = (cat || '').toLowerCase();
    const subLower = (sub || '').toLowerCase();
    
    // Check if it's a gender-specific category
    const isWomenCat = catLower === 'women' || subLower.includes('women') || subLower.includes('ladies') || subLower.includes('girl');
    const isMenCat = catLower === 'men' && !isWomenCat;
    
    if (!isWomenCat && !isMenCat) return; // Neutral category — no filter needed
    
    const grid = document.getElementById('shop-grid') || document.getElementById('product-grid');
    if (!grid) return;

    const cards = grid.querySelectorAll('.product-card[onclick]');
    let hiddenCount = 0;
    ...
    cards.forEach(card => {
      ...
      if (!match) return;
      const pid = parseInt(match[0]);
      const all = [...(window.products || []), ...(window.allProducts || [])];
      const p = all.find(x => x.id === pid);
      if (!p) return;
      
      const pCat = (p.category || '').toLowerCase();
      
      if (isWomenCat && (pCat === 'men' && pCat !== 'women')) {
        card.style.display = 'none';
        hiddenCount++;
      } else if (isMenCat && (pCat === 'women' && pCat !== 'men')) {
        card.style.display = 'none';
        hiddenCount++;
      }
    });
    ...
    if (hiddenCount > 0) {
      console.log(`[GenderFilter] ${hiddenCount} cross-gender products hidden from "${cat}" category`);
    }
  }

  /* ──────────────────────────────────────────────────────────────
     FIX 2 — DUPLICATE IMAGE AUTO-DETECT & REMOVE
  ────────────────────────────────────────────────────────────── */
  function _deduplicateProductImages() {
    const all = [...(window.products || []), ...(window.allProducts || [])];
    all.forEach(p => {
      if (!Array.isArray(p.imgs)) return;
      const seen = new Set();
      p.imgs = p.imgs.filter(url => {
        if (!url) return false;
      ...
      if (p.img && !p.imgs.includes(p.img)) p.img = p.imgs[0] || p.img;
    });
  }

  /* ──────────────────────────────────────────────────────────────
     FIX 3 — COMBO PDP LAYOUT (Screenshot 1 ki tarah)
     Best Friend/Couple/Matching combos ke liye vertical thumbnail
  ────────────────────────────────────────────────────────────── */
  function _isComboProduct(p) {
    if (!p) return false;
    const name = (p.name || '').toLowerCase();
    const cat = (p.category || '').toLowerCase();
    const sub = (p.sub || '').toLowerCase();
    return cat === 'combos' || sub.includes('combo') || name.includes('couple') ||
           name.includes('matching') || name.includes('best friend') || name.includes('combo') ||
           p.is_combo === true;
  }

  function _buildComboPDPLayout(product) {
    const imgs = Array.isArray(product.imgs) ? product.imgs : (product.img ? [product.img] : []);
    if (!imgs.length) return;
    
    // PDP image container dhundho
    const pdpImg = document.getElementById('pdp-main-image') || document.querySelector('#pdp-container img');
    if (!pdpImg) return;
    
    const container = pdpImg.closest('.relative') || pdpImg.parentElement;
    if (!container || container.dataset.comboBuilt) return;
    container.dataset.comboBuilt = '1';
    
    // Purana image hide karo
    pdpImg.style.display = 'none';
    
    // Combo layout inject karo
    const wrap = document.createElement('div');
    wrap.id = 'ok-combo-pdp-wrap';
    
    // Combo badge
    const comboBadge = `<div class="ok-combo-badge">🎁 Combo Set</div>`;
    
    // Thumbnails (left column)
    const thumbsHTML = imgs.map((img, i) => 
      `<img src="${img}" class="ok-combo-thumb ${i===0?'active':''}" data-idx="${i}" 
           onclick="window._okComboThumbClick(${i})"
           onerror="this.style.display='none'" alt="combo ${i+1}">`
    ).join('');
    
    wrap.innerHTML = `
      ${comboBadge}
      <div id="ok-combo-thumb-col">${thumbsHTML}</div>
      <img id="ok-combo-main-img" src="${imgs[0]}" alt="${product.name}"
           onclick="window.openLightbox && window.openLightbox(0)"
           onerror="this.src='https://placehold.co/300x380/f3f4f6/9ca3af?text=Combo'">
    `;
    
    container.style.padding = '0';
    container.appendChild(wrap);
    
    // Store imgs on window for lightbox
    window._comboPdpImgs = imgs;
    
    window._okComboThumbClick = function(idx) {
      const mainImg = document.getElementById('ok-combo-main-img');
      if (mainImg) { mainImg.style.opacity = '0'; setTimeout(() => { mainImg.src = imgs[idx]; mainImg.style.opacity = '1'; }, 150); }
      document.querySelectorAll('.ok-combo-thumb').forEach((t, i) => t.classList.toggle('active', i === idx));
    };
    
    console.log('[ComboPDP] ✅ Vertical thumbnail layout built for combo product');
  }

  /* ──────────────────────────────────────────────────────────────
     FIX 4 — DYNAMIC PRICING (Screenshot 2 ki tarah)
     COD select karo → price +₹19
     UPI select karo → base price
  ────────────────────────────────────────────────────────────── */
  const COD_SURCHARGE = 19;
  let _currentPaymentMode = 'upi'; // default UPI
  let _currentBasePrice = 0;

  function _injectPaymentToggle(product) {
    document.getElementById('ok-payment-toggle')?.remove();
    
    _currentBasePrice = product.price || 0;
    _currentPaymentMode = 'upi';
    
    // PDP price element dhundho
    const priceEl = document.querySelector('#pdp-container .text-3xl.font-bold, #pdp-container [id*="pdp-price"], #pdp-container .text-2xl.font-black');
    if (!priceEl) return;
    
    // Live price span ID lagao
    priceEl.id = 'ok-pdp-live-price';
    
    const toggle = document.createElement('div');
    toggle.id = 'ok-payment-toggle';
    toggle.innerHTML = `
      <div class="ok-pt-label">💳 Payment Mode Chuniye</div>
      <div class="ok-pt-options">
        <button class="ok-pt-btn active" id="ok-pt-upi" onclick="window._okSelectPayment('upi')">
          <span>📱 UPI / Online</span>
          <span class="ok-pt-badge">₹${_currentBasePrice.toLocaleString('en-IN')} — Best Price</span>
        </button>
        <button class="ok-pt-btn" id="ok-pt-cod" onclick="window._okSelectPayment('cod')">
          <span>💵 Cash on Delivery</span>
          <span class="ok-pt-badge">₹${(_currentBasePrice + COD_SURCHARGE).toLocaleString('en-IN')} (+₹${COD_SURCHARGE})</span>
        </button>
      </div>
      <div class="ok-cod-surcharge-note hidden" id="ok-cod-note">
        ⚠️ COD pe ₹${COD_SURCHARGE} extra charge hai
      </div>
    `;
    
    // Price element ke baad insert karo
    const pdpPriceSection = priceEl.closest('.px-4, .p-4') || priceEl.parentElement;
    pdpPriceSection?.insertAdjacentElement('afterend', toggle);
    
    window._okSelectPayment = function(mode) {
      _currentPaymentMode = mode;
      const upiBtn = document.getElementById('ok-pt-upi');
      const codBtn = document.getElementById('ok-pt-cod');
      const codNote = document.getElementById('ok-cod-note');
      const livePrice = document.getElementById('ok-pdp-live-price');
      ...
      if (mode === 'cod') {
        ...
        if (livePrice) {
          livePrice.textContent = '₹' + (_currentBasePrice + COD_SURCHARGE).toLocaleString('en-IN');
          livePrice.style.color = '#f59e0b';
        }
        // Update Add to Cart button with COD info
        const cartBtn = document.getElementById('pdp-add-cart') || document.querySelector('#pdp-container button[onclick*="addToCart"]');
        if (cartBtn) { window._pdpCurrentPayMode = 'cod'; window._pdpCODSurcharge = COD_SURCHARGE; }
      } else {
        codBtn?.classList.remove('active');
        upiBtn?.classList.add('active');
        codNote?.classList.add('hidden');
        if (livePrice) {
          livePrice.textContent = '₹' + _currentBasePrice.toLocaleString('en-IN');
          livePrice.style.color = '';
        }
        window._pdpCurrentPayMode = 'upi';
        window._pdpCODSurcharge = 0;
      }
    };
  }

  /* ──────────────────────────────────────────────────────────────
     FIX 5 — "YOU MIGHT ALSO LIKE" SECTION (Screenshot 3 ki tarah)
  ────────────────────────────────────────────────────────────── */
  function _injectYouMightAlsoLike(product) {
    document.getElementById('ok-ymla-section')?.remove();
    
    const category = product.category;
    const all = [...(window.products || []), ...(window.allProducts || [])];
    
    // Same category ke products dhundho, current product exclude karo
    let recs = all.filter(p => 
      p.id !== product.id && 
      p.category === category && 
      p.is_active !== false
    );
    ...
    // Shuffle + take 8
    recs = recs.sort(() => Math.random() - 0.5).slice(0, 8);
    if (!recs.length) return;
    
    const section = document.createElement('div');
    section.id = 'ok-ymla-section';
    
    const cardsHTML = recs.map(p => {
      const img = p.imgs?.[0] || p.img || 'https://placehold.co/130x150/f3f4f6/9ca3af?text=?';
      const isGold = !!(p.is_gold || (window.goldProducts || []).find(g => g.id === p.id));
      return `<div class="ok-ymla-card" onclick="openProductPage(${p.id}${isGold ? ', true' : ''})">
        <img src="${img}" alt="${p.name}" loading="lazy" onerror="this.src='https://placehold.co/130x150/f3f4f6/9ca3af?text=?'">
        <div class="ok-ymla-card-info">
          ${isGold ? '<div style="font-size:8px;font-weight:900;color:#B8860B;margin-bottom:2px;">⭐ GOLD</div>' : ''}
          <div class="ok-ymla-card-name">${p.name}</div>
          <div class="ok-ymla-card-price">₹${(p.price||0).toLocaleString('en-IN')}
            ${p.oldprice && p.oldprice > p.price ? `<span style="text-decoration:line-through;font-size:9px;color:#aaa;font-weight:400;margin-left:3px;">₹${p.oldprice}</span>` : ''}
          </div>
        </div>
      </div>`;
    }).join('');
    ...
    section.innerHTML = `
      <h3>
        <i class="fas fa-bolt" style="color:#e11d48;font-size:14px;"></i>
        You Might Also Like
        <span class="ok-ymla-ad-badge" style="margin-left:auto;font-size:9px;color:#9ca3af;font-weight:700;">Sponsored</span>
      </h3>
      <div class="ok-ymla-scroll">${cardsHTML}</div>
    `;
    
    // PDP container ke end mein lagao
    const pdpContainer = document.getElementById('pdp-container') || document.getElementById('view-product');
    if (pdpContainer) {
      const reviewSection = pdpContainer.querySelector('[id*="review"], [id*="rating"]');
      if (reviewSection) reviewSection.insertAdjacentElement('afterend', section);
      else pdpContainer.appendChild(section);
    }
  }

  /* ──────────────────────────────────────────────────────────────
     FIX 6 — POPULAR CATEGORIES on Category Page
  ────────────────────────────────────────────────────────────── */
  const POPULAR_CATS = [
    { label: 'Oversized Tees', cat: 'Men', sub: 'Oversized Tees', img: 'https://images.unsplash.com/photo-1583743814966-8936f5b7be1a?w=120&h=120&fit=crop&q=80', trending: true },
    { label: "Women's Kurtis", cat: 'Women', sub: 'Kurtis', img: 'https://images.unsplash.com/photo-1582718560869-01152e38cfd4?w=120&h=120&fit=crop&q=80', trending: true },
    { label: 'Cargo Pants', cat: 'Men', sub: 'Cargo Pants', img: 'https://images.unsplash.com/photo-1617196034183-421b4040ed20?w=120&h=120&fit=crop&q=80', trending: false },
    { label: 'Dresses', cat: 'Women', sub: 'Dresses', img: 'https://images.unsplash.com/photo-1572804013309-59a88b7e92f1?w=120&h=120&fit=crop&q=80', trending: true },
    { label: 'Sneakers', cat: 'Men', sub: 'Sneakers', img: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=120&h=120&fit=crop&q=80', trending: false },
    { label: "Women's Watches", cat: 'Accessories', sub: 'Watches', img: 'https://images.unsplash.com/photo-1524592094714-0f0654e20314?w=120&h=120&fit=crop&q=80', trending: false },
    { label: "Men's Watches", cat: 'Accessories', sub: 'Watches', img: 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=120&h=120&fit=crop&q=80', trending: false },
    { label: 'Combos', cat: 'Combos', sub: 'Couple Combo', img: 'https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=120&h=120&fit=crop&q=80', trending: true },
  ];

  function _injectPopularCategories() {
    const catPage = document.getElementById('view-categories');
    if (!catPage) return;
    if (document.getElementById('ok-popular-cats')) return;
    
    const section = document.createElement('div');
    section.id = 'ok-popular-cats';
    
    const itemsHTML = POPULAR_CATS.map(c => `
      <div class="ok-popcat-item" onclick="openSubcatProducts('${c.cat}','${c.sub.replace(/'/g,"\\'")}');window._closeCategories&&window._closeCategories()">
        <div class="ok-popcat-img-wrap">
          <img src="${c.img}" alt="${c.label}" loading="lazy" onerror="this.style.display='none'">
          ${c.trending ? '<span class="ok-popcat-trend-badge">🔥</span>' : ''}
        </div>
        <div class="ok-popcat-label">${c.label}</div>
      </div>
    `).join('');
    
    section.innerHTML = `
      <h3><i class="fas fa-fire" style="color:#e11d48;"></i> Popular Right Now</h3>
      <div class="ok-popcats-scroll">${itemsHTML}</div>
    `;
    
    // Category page ke top mein insert karo
    const existingContent = catPage.querySelector('.flex, #ok-csb') || catPage.firstElementChild;
    if (existingContent) catPage.insertBefore(section, existingContent);
    else catPage.appendChild(section);
  }

  /* ──────────────────────────────────────────────────────────────
     FIX 7 — SMART BACK NAVIGATION
     Category > Sub-category > PDP → Back = Sub-category page
  ────────────────────────────────────────────────────────────── */
  function _patchSmartBackNav() {
    if (window._smartBackPatched) return;
    window._smartBackPatched = true;

    // openProductPage wrap karo — navigation history save karo
    const origOpenPP = window.openProductPage;
    if (origOpenPP) {
      window.openProductPage = async function(id, isGold) {
        // Save state before opening PDP
        window._pdpBackState = {
          view: window.currentView || 'home',
          cat: window._currentCatFilter || null,
          sub: window._currentSubFilter || null,
        };
        return origOpenPP(id, isGold);
      };
    }

    // PDP back button patch karo
    const _patchPDPBackBtn = () => {
      const backBtns = document.querySelectorAll('#view-product .back-btn, #pdp-container .back-btn, [onclick*="navigate(\'shop\')"], [onclick*="navigate(\'home\')"]');
      backBtns.forEach(btn => {
        if (btn.dataset.smartBackDone) return;
        btn.dataset.smartBackDone = '1';
        btn.onclick = function(e) {
          e.preventDefault();
          e.stopPropagation();
          const state = window._pdpBackState;
          if (state && state.sub && state.cat && typeof window.openSubcatProducts === 'function') {
            // Sub-category page pe wapas jao
            navigate('shop');
            setTimeout(() => {
              window.openSubcatProducts(state.cat, state.sub);
            }, 150);
    ...
    new MutationObserver(() => {
      ...
      if (pdp && !pdp.classList.contains('hidden')) {
        setTimeout(_patchPDPBackBtn, 300);
      }
    }).observe(document.body, { subtree: true, attributes: true, attributeFilter: ['class'] });
    
    setTimeout(_patchPDPBackBtn, 500);
  }

  /* ──────────────────────────────────────────────────────────────
     PDP OPEN HOOK — Combo layout + payment toggle + YMLA inject
  ────────────────────────────────────────────────────────────── */
  function _patchOpenProductPageForV7() {
    if (window._v7PdpPatched) return;
    window._v7PdpPatched = true;

    const origOpenPP = window.openProductPage;
    if (!origOpenPP) return;
    ...
    window.openProductPage = async function(id, isGold) {
      ...
      const result = await origOpenPP(id, isGold);
      setTimeout(() => {
        const all = [
          ...(window.products || []),
          ...(window.allProducts || []),
        ...
        const product = all.find(p => p.id === id || String(p.id) === String(id));
        if (!product) return;
        
        // Deduplicate images
        if (Array.isArray(product.imgs)) {
          const seen = new Set();
          product.imgs = product.imgs.filter(u => { const k = (u||'').trim(); if (seen.has(k)) return false; seen.add(k); return true; });
        }
        
        // Combo layout
        if (_isComboProduct(product)) {
          _buildComboPDPLayout(product);
        }
        
        // Payment toggle
        _injectPaymentToggle(product);
        
        // You Might Also Like
        setTimeout(() => _injectYouMightAlsoLike(product), 400);
  ...
  /* ──────────────────────────────────────────────────────────────
     INIT — Saari fixes apply karo
  ────────────────────────────────────────────────────────────── */
  function _initV7() {
    // 1. Gender filter
    _patchStrictGenderFilter();
    
    // 2. Deduplicate existing product images
    setTimeout(_deduplicateProductImages, 1500);
    
    // 3. Smart back navigation
    _patchSmartBackNav();
    
    // 4. PDP features (combo + pricing + YMLA)
    const _waitForOpenPP = setInterval(() => {
      if (typeof window.openProductPage === 'function') {
        clearInterval(_waitForOpenPP);
        _patchOpenProductPageForV7();
      }
    }, 200);
    
    // 5. Popular Categories on category page
    new MutationObserver(() => {
      const catPage = document.getElementById('view-categories');
      if (catPage && !catPage.classList.contains('hidden') && !document.getElementById('ok-popular-cats')) {
        setTimeout(_injectPopularCategories, 300);
      }
    }).observe(document.body, { subtree: true, attributes: true, attributeFilter: ['class'] });
    setTimeout(_injectPopularCategories, 1000);
    
    // 6. Patch navigate to also track cat/sub
    const origNav = window.navigate;
    if (origNav && !window._v7NavPatched) {
      window._v7NavPatched = true;
      window.navigate = function(view, ...args) {
        if (view === 'categories') {
          setTimeout(_injectPopularCategories, 500);
        }
        return origNav(view, ...args);
      };
    }

    console.log('%c🎁 OutfitKart V7 ✅ Combo+COD+YMLA+PopCats+GenderFilter+SmartBack all loaded!', 
      'background:#7c3aed;color:white;font-weight:900;font-size:11px;padding:4px 12px;border-radius:5px;');
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(_initV7, 700));
  } else {
    setTimeout(_initV7, 700);
  }
